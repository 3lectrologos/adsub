...
\todo{Add adaptive submodular maximization under cardinality constraints definition.}\\
\todo{Add definitions of adaptive monotonicity and submodularity.}\\
\todo{Need to assume that $n \geq 2k$ by adding dummy elements, so that it's always possible to choose $k$ of them with non-negative expected marginal gains.}\\
...
\begin{proof}
  ...
  \begin{align*}
    ...
    \Leftrightarrow&\ \favg(\pigi) \geq \left(1 - \left(1 - \frac{1}{k}\right)^{i}\right)\favg(\pik)\tag*{(by non-negativity)}\\
