...
\begin{proof}
  ...
  Let $p(\psi'\mmid\psi)$ be the probability that partial realization $\psi' \supseteq \psi$ will come up when running policy $\pik$. Then, the gain $\D{\pik}{\psi}$ can be bounded as follows:
  \begin{align*}
...
\end{lemma}
\begin{proof}
  Let $|E| = n$. For any $\phi \in O^E$, let $(e_1(\phi), e_2(\phi), \dots, e_n(\phi))$ be an ordering such that $p \geq p_1(\phi) \geq p_2(\phi) \geq \dots \geq p_n(\phi)$, where $p_i(\phi) \defeq \P[\Pi]{e_i \in E(\pi, \phi)},\ \forall i \in [n]$. We also define $E_i(\phi) \defeq \{e_1(\phi), \ldots, e_i(\phi)\}, \forall i \in [n]$ with $E_0(\phi) \defeq \varnothing$. Furthermore, let $X_i$ be a random variable that indicates whether an element is or is not selected by policy $\pi$:
\begin{align*}
  X_i(\phi) = \twopartdefo{1}{e_i \in E(\pi, \phi)}{0}.
\end{align*}
Then,
  \begin{align*}
    \favg(\pi) &= \E[\Phi,\Pi]{f(E(\pi, \Phi), \Phi)}\\
               &= \E[\Phi,\Pi]{f(\varnothing, \Phi) + \sum_{i=1}^{|E(\pi, \Phi)|}\left(f(E_i(\Phi) \cap E(\pi, \Phi), \Phi) - f(E_{i-1}(\Phi) \cap E(\pi, \Phi), \Phi)\right)}\\
               &= \E[\Phi,\Pi]{f(\varnothing, \Phi) + \sum_{i=1}^{n}X_i(\Phi)\left(f(E_i(\Phi) \cap E(\pi, \Phi), \Phi) - f(E_{i-1}(\Phi) \cap E(\pi, \Phi), \Phi)\right)}\\
               &\geq \E[\Phi,\Pi]{f(\varnothing, \Phi) + \sum_{i=1}^{n}X_i(\Phi)\left(f(E_i(\Phi), \Phi) - f(E_{i-1}(\Phi), \Phi)\right)} \tag*{(by submodularity)}\\
               &= \E[\Phi]{f(\varnothing, \Phi) + \sum_{i=1}^{n}\E[\Pi]{X_i(\Phi)}\left(f(E_i(\Phi), \Phi) - f(E_{i-1}(\Phi), \Phi)\right)}\\
               &= \E[\Phi]{f(\varnothing, \Phi) + \sum_{i=1}^{n}p_i\left(f(E_i(\Phi), \Phi) - f(E_{i-1}(\Phi), \Phi)\right)}\\
               &= \E[\Phi]{(1-p_1)f(\varnothing, \Phi) + \sum_{i=1}^{n}(p_i - p_{i+1})f(E_i(\Phi), \Phi) + p_nf(E_n(\Phi), \Phi)}\\
...
\begin{cor}
  For any policy $\pi$ such that each element of the ground set is selected by it with probability at most $p$, that is, $\P[\Pi]{e \in E(\pi, \phi)} \leq p,\ \forall \phi \in O^E,\ \forall e \in E$, if $f(\cdot\,, \phi)$ is submodular for all $\phi \in O^E$, then the gain of running $\pi$ after any policy $\pi'$ can be bounded as follows:
\begin{align*}
