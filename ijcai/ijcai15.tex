...
\newcommand{\subflen}{0.34\textwidth}
...
\begin{figure*}
    ...
    \begin{minipage}[b][0.5\textwidth]{0.45\textwidth}
      \setlength\figureheight{0.52\textwidth}
      \setlength\figurewidth{0.82\textwidth}
      \begin{subfigure}[b]{\textwidth}
        \centering
        \input{figures/pinf_ego_fb.tex}
        %\vspace{\scspacey}
        \caption{\hspace{\scspacex}\infmax{} -- \fbook}
...
Furthermore, if our goal is to reach a specific level of objective value using as few nodes as possible, then our gains due to adaptivity can be even more substantial in terms of the number of required nodes.
For example, as shown in \figref{fig:exp2}\subcapref{fig:fs_ego_fb}, if we want to attain a maximum cut objective value of $1900$ for the \fbook network, a budget $k$ of about $13$ nodes is enough for adaptive random greedy, while a budget of almost $30$ nodes is required for non-adaptive random greedy.

For the other two plots of \figref{fig:exp2} we fix $k=20$.
...
A value of $\beta=0$ corresponds to the setting we used in \figref{fig:exp1}, where the cutting probability is uniformly distributed among the selected node and each of its neighbors; $\beta=1$ corresponds to deterministically cutting the selected node.
