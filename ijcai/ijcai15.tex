...
However, \citet{golovin11} showed that the notions of monotonicity and submodularity can be naturally generalized to this setting, and be used to give similar performance guarantees as in the non-adaptive case.
...
Then, following properties can be defined analogously to their non-adaptive counterparts:
\begin{itemize}
...
We now present our proposed adaptive random greedy policy ($\pig$) for maximizing (non-monotone) adaptive submodular objectives.
...
\begin{algorithm}[!b]
  ...
  \normalsize{
    \begin{algorithmic}[1]
      ...
      \LET{$\mathcal{M}_k(\psi)$}{$\displaystyle\argmax_{S \subseteq E \setminus A,\,|S| = k}\left\{\sum_{e \in S} \D{e}{\psi} \right\}$} \label{lin:upd1}
      \STATE Sample $m$ uniformly at random from $\mathcal{M}_k(\psi)$ \label{lin:upd2}
      \LET{$A$}{$A \cup \{m\}$}
      \STATE Observe $\Phi(m)$
...
\clearpage
\section{Experiments}
We evaluated our proposed algorithm on two objective functions using both synthetic and real-world data sets.
Since we have no competitor policy for the adaptive non-monotone submodular setting, we rather focus here on showcasing the potential benefits of adaptivity by comparing adaptive to non-adaptive random greedy on two objectives that can be formulated in both settings.

\subsection{Influence maximization}
The problem of influence maximization in a social network was posed by \citet{kempe03}.
Given a graph and a model of influence propagation, e.g., independent cascades, the goal is to find a subset of nodes, which will maximize the expected number of nodes that will be ultimately influenced according to the propagation model.
\citet{kempe03} showed that this objective is monotone submodular.
\cite{golovin11} formulated an adaptive version of this problem, where the selection of a node $v$ reveals the status (active or not active) of all outgoing edges of $v$ and of any other node that can be reached from $v$.

Here, we consider a modular cost term $|A|$, that is, each select incurs a unit cost.
This term is subtracted from the original expected influence functions resulting in a new normalized, non-monotone, submodular objective for any realization, which satisfies our assumptions for both the non-adaptive and the adaptive settings.

\subsection{Maximum graph cut}
...
Interestingly, the maximum cut objective is normalized, symmetric submodular, and, therefore, non-monotone.

Here, we consider an adaptive version of this problem, in which selecting a node results in cutting that node or either of its neighbors with equal probability.
This objective is also normalized, non-monotone, and submodular for any realization.
