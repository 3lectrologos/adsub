...
\begin{abstract}
Following the widespread success of monotone submodular functions and the greedy algorithm in modeling and solving a wide range of combinatorial problems, non-monotone submodular functions have been receiving increasing attention due to their occurence in real-world applications.
At the same time, the work on sequential stochastic optimization, where decisions are made adaptively based on uncertain observations, has led to the generalization of the concepts of monotonicity and submodularity to their adaptive analogues, and the introduction of the adaptive greedy algorithm with similar approximation guarantees to its non-adaptive counterpart.
Based on the recently proposed random greedy algorithm for non-monotone submodular maximization, we propose an adaptive version thereof, and prove that it retains the $1-1/e$ approximation ratio of adaptive greedy for adaptive monotone submodular objectives, while it additionally obtains a $1/e$ approximation ratio for non-monotone objectives under a slightly stronger condition than adaptive submodularity.
We showcase the benefits of adaptivity on several small-scale network data sets using two popular non-monotone objectives.
\end{abstract}
...
Note that $f$ here is neither normalized nor monotone.
...
\citet{kempe03} showed that this function is normalized, monotone, and submodular, for any realization $\Phi$.
...
\end{align*}
which results in a new normalized, non-monotone, submodular objective for any realization $\Phi$.
...
\end{align*}
The new objective is also normalized, non-monotone, and submodular, for any realization $\Phi$.
