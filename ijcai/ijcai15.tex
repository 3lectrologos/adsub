...
If $f$ is adaptive monotone, we may use the bound $\favg(\pigi @ \pi) \geq \favg(\pi)$ and, more specifically, $\favg(\pigi @ \pik) \geq \favg(\pik)$, which readily follow from the definition of adaptive monotonicity, to obtain the following theorem.
\begin{theorem}
...
As a consequence, we get that $\favg(\pigi@\pi) = \favg(\pi@\pigi) \geq (1-p)\favg(\pi)$, meaning that the elements added by adaptive random greedy cannot reduce the average value obtained by any other policy $\pi$ by too much.
