...
Various techniques have been proposed to make the computation of marginal gains feasible for large-scale networks \cite{cohen14,ohsaka14}.
In this paper we chose to run experiments on smaller-scale networks, but these techniques can also be applied to scale up adaptive random greedy.
In the same context, \citet{he14} recently introduced the problem of influence difference maximization, which interestingly results in maximizing a non-monotone submodular objective.

Maximum cut has been a much-studied NP-complete problem with provable approximation guarantees given by SDP-based algorithms for both the unconstrained \cite{goemans95} and cardinality-constrained \cite{feige01} problems.
An interesting application of maximum cut objectives has been proposed by \citet{lin11} for text summarization.
