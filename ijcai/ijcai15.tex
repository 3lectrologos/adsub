...
In our birdwatching example, the ground set $E = \{\ell_1, \ell_2$, $\ell_3\}$ consists of the three possible locations we may visit, and the state of each location is the (random) subset of the available bird species that we actually observe if we travel to that location; each realization maps locations to subsets of actually observed species.
Then, our objective function is the size of the union of actually observed species given a subset of visited locations.
...
Our birdwatching policy would specify which location to visit next (or, more generally, a distribution of next locations), given the already visited locations and the already observed species in each of them.
...
That is, find a policy that maximizes the expected number of observed bird species, if we can visit at most $k$ locations.
