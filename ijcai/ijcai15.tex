...
\newcommand{\am}[1]{{\textbf{[Amin: #1]}}\xspace}
\newcommand{\ak}[1]{{\textbf{[AK: #1]}}\xspace}
%-------------------------------------------------------------------------------
...
\begin{abstract}
A wide range of AI problems require sequentially selecting elements from a large set with the goal of optimizing the utility of the selected subset. 
...
However, when the objective function is adaptive monotone and adaptive submodular, a simple greedy policy attains a $1-1/e$ approximation ratio to the optimum policy in terms of expected utility.
Unfortunately, many practical objective  functions are naturally non-monotone; \am{this simple example may not be very clear at this point} as a simple example, each selected element may incur some cost that needs to be subtracted from the original monotone utility, resulting in an non-monotone objective.
...
We propose the \emph{adaptive random greedy policy} \am{choose a name for the algorithm} for maximizing adaptive submodular functions, and prove that it retains the aforementioned $1-1/e$ approximation guarantee for a functions that are adaptive submodular and   adaptive monotone, while it additionally provides a $1/e$ approximation guarantee  for non-monotone adaptive submodular functions.
\am{not clear what you mean}We showcase the benefits of our proposed policy on several network data sets using two non-monotone functions, representative of two classes of commonly encountered non-monotone objectives.
\end{abstract}
...
\begin{figure}[tb]
  ...
  \begin{tikzpicture}[
      ...
      >=latex,
      line join=bevel,
      LN/.style={draw=gray!80!black,rectangle,line width=0.1em,minimum size=2em},
    ...
    \node[LN] (l3) at (75bp,0bp) {$\ell_3$};
    \node[BN] (b1) at (-200bp,-75bp) {\includegraphics[width=0.5cm,trim=0 20 0 20,clip]{figures/swallow.png}};
    \node[BN] (b2) at (-150bp,-75bp) {\includegraphics[width=0.5cm,trim=0 20 0 20,clip]{figures/hawk.png}};
    \node[BN] (b3) at (-100bp,-75bp) {\includegraphics[width=0.5cm,trim=0 20 0 20,clip]{figures/dove.png}};
    \node[BN] (b4) at (-50bp,-75bp) {\includegraphics[width=0.5cm,trim=0 20 0 20,clip]{figures/crow.png}};
    \node[BN] (b5) at (0bp,-75bp) {\includegraphics[width=0.5cm,trim=0 20 0 20,clip]{figures/thrush.png}};
    \node[BN] (b6) at (50bp,-75bp) {\includegraphics[width=0.5cm,trim=0 20 0 20,clip]{figures/duck.png}};
    \node[BN] (b7) at (100bp,-75bp) {\includegraphics[width=0.5cm,trim=0 20 0 20,clip]{figures/shorebird.png}};
    ...
    \draw[LL] (l1) -- (-150bp,-52.5bp);
    \draw[LL] (l2) -- (fit2);
    \draw[LL] (l3) -- (fit3);
  \end{tikzpicture}
  \caption{Birdwatching maximum cover example. \am{it is not cleat what $l_1,l_2,l_3$ point to} \ak{sets cut through circles..}}\label{fig:toy}
\end{figure}

Many practical problems in artificial intelligence boil down to selecting a number of elements from a large set of options in an initially unknown environment, so as to maximize some utility function defined over subsets of selected elements. \ak{Examples include ...}
In contrast to the {\em non-adaptive} setting, where we commit to the selected elements all at once, in the {\em adaptive} setting the selection process is performed in a sequential manner, and each element that is picked provides some form of stochastic feedback, or, in other words, reveals part of the environment.
...
Suppose that triangle \ak{no triangles?} nodes represent birdwatching locations; by visiting each of them we observe a random number of bird species among those that are known to exist in that location.\footnote{\url{http://www.allaboutbirds.org/}}
We would like to plan a birdwatching trip to visit a number of these locations and to maximize the number of observed species.
It is intuitive that taking into account the already observed species and making  decisions about which place to visit next can be greatly beneficial \am{what is beneficial?} compared to committing to the full trip in advance.
...
Most notably, \citet{golovin11} showed that when the objective function under consideration is adaptive monotone and adaptive submodular, then a simple greedy policy attains a $1-1/e$ approximation ratio in terms of expected utility. \am{$1-1/e$ approximation ratio does not have any meaning. You have used it many times.  it is approximation guarantee to the optimal algorithm/best policy, etc}

While adaptive submodularity and its intuitive meaning of ``diminishing returns'' is fundamental to a number of objective functions of interest in the adaptive setting, the additional assumption of adaptive monotonicity \ak{explain} is often not satisfied in practice.
In our birdwatching example, the original objective is adaptive monotone submodular \am{I think we do not have this term. a function is adaptive monotone and/or adaptive submodular, but apdative monotone submod. is weird}; however, if each subset of locations has an associated cost, e.g., the total required travel distance to visit all of them, our new objective will naturally be non-monotone.
...
To resolve this, we propose a new policy for maximizing non-monotone adaptive submodular functions, called {\em adaptive random greedy}.
...
Each item $e \in E$ is associated with a state $o \in O$ through a function $\phi : E \to O$, which is called a {\em realization} of the set of states.
We also assume that the realization $\Phi$ is a random variable with known distribution $p(\phi)$ \ak{$\Phi$ vs $\phi$?}.
...
To address the above complications, we define a {\em partial realization} as a set $ \psi \subseteq E \times O$, which represents the item-observation pairs over a subset of $E$.
...
We can now define a \emph{policy} $\pi$ as a function from partial realizations to a distribution $\mathcal{P}(E)$ over items, specifying  which item to pick next, formally, $\pi : 2^{E \times O} \to \mathcal{P}(E)$ \am{I changed the meaning of a  policy, I think this is what you meant}. 
...
We also use the shorthand notation $\pi(e\mmid\psi)$ for the probability of picking an item $e$ given the partial realization $\psi$.
...
Note that this set is a random variable %\am{strictly speaking, a set cannot be a random variable} 
that depends on both the randomness of the policy, as well as the randomness of realizations.
...
In this setting, the marginal gain of an element $e \in E$ given a subset $B \subseteq E$ is defined as $f(B \cup \{e\}) - f(B)$.
...
Function $f$ is called {\em monotone} if, for any $B \subseteq C \subseteq E$, it holds that $f(B) \leq f(C)$, which is equivalent to saying that the marginal gain is always non-negative.
Furthermore, $f$ is called {\em submodular}, if, for any $B \subseteq C \subseteq E$ and any $e \in E \setminus C$, it holds that $f(C \cup \{e\}) - f(C) \leq  f(B \cup \{e\}) - f(B)$.
This means that the marginal gain of any element decreases as the given set increases ($C \supseteq B$); in other words, submodularity expresses the ``diminishing returns'' property as more and more elements are added to our selection.

In their seminal work, \citet{nemhauser78} showed that, if $f$ is non-negative, monotone, and submodular, then constructing a subset $A$ by picking elements greedily according to their marginal gains, guarantees that $f(A)$ is a ($1 - 1/e$)-approximation of the optimal value (\am{under cardinality constraint}).
...
More concretely, the {\em expected marginal gain} of an element $e \in E$ given partial realization $\psi$ can be defined as
\begin{align*}
...
\begin{itemize}
\item $f$ is called {\em adaptive monotone}, if $\D{e}{\psi} \geq 0$, for all $e \in E$ and all $\psi$ of positive probability,
\item $f$ is called {\em adaptive submodular}, if $\D{e}{\psi'} \leq \D{e}{\psi}$, for all $e \in E \setminus \dom(\psi')$ and all $\psi' \supseteq \psi$.
\end{itemize}
...
\citet{golovin11} showed that, if $f$ is adaptive monotone submodular \am{see before}, then adaptive greedy is a $(1-1/e)$-approximation of the optimal policy in terms of expected utility $\favg$ \am{adaptive greedy is a policy, its utility is a $(1-1/e)$-approximation to the opt}.
...
We now present our proposed {\em adaptive random greedy policy} ($\pig$) for maximizing adaptive submodular functions, and prove approximation ratios {\em irrespective} of whether adaptive monotonicity is satisfied or not.
...
The key difference compared to the original adaptive greedy policy is shown in \linesref{lin:upd1}{lin:upd2}; rather than selecting the element with the largest expected marginal gain, $\pig$ randomly selects an element among the ones with the $k$ largest gains \ak{Define and refer to $\Mk(\psi)$?}.
...
Our proofs generalize the results of \citet{buchbinder14} for the random greedy algorithm, and are presented in detail in the long version of this paper\footnote{\todo{add link}}. Below, we provide an outline of our analysis.
...
Furthermore, although this is not visible in \figref{fig:exp}, if our goal is to reach a level of objective value, e.g., cut $1000$ edges, using as few nodes as possible, then the gains of adaptivity in terms of nodes required can be even more considerable \ak{quantify?}.
...
Example application domains, apart from those we present in this paper, include active learning \cite{chen14} and incentive mechanism design \cite{singla13}. \ak{might want to cite Guillory and Bilmes Interactive Submodular Cover}
